...
class EpsonFiscalDriver:
    ...
    fiscalStatusErrors = [#(1<<0 + 1<<7, "Memoria Fiscal llena"),
                          ...
                          (1<<1, "Error de comprobacion en memoria de trabajo"),
                          (1<<2, "Poca bateria"),
                          (1<<3, "Comando no reconocido"),
                          (1<<4, "Campo de datos no valido"),
                          (1<<5, "Comando no valido para el estado fiscal"),
                          (1<<6, "Desbordamiento de totales"),
                          (1<<7, "Memoria Fiscal llena"),
                          (1<<8, "Memoria Fiscal casi llena"),
                          (1<<11, "Es necesario hacer un cierre de la jornada fiscal o se supero la cantidad maxima de tickets en una factura."),
                          ]

    printerStatusErrors = [(1<<2, "Error y/o falla de la impresora"),
                          (1<<3, "Impresora fuera de linea"),
##                          (1<<4, "Poco papel para la cinta de auditoria"),
##                          (1<<5, "Poco papel para comprobantes o tickets"),
    ...
    def _incrementSequenceNumber( self ):
        # Avanzo el numero de sequencia, volviendolo a 0x20 si paso el limite
    ...
    def _sendMessage( self, message ):
        ...
        while 1:
            if time.time() > timeout:
                raise ComunicationError, "Expiro el tiempo de espera para una respuesta de la impresora. Revise la conexion."
            c = self._read(1)
            if len(c) == 0:
                continue
            ...
            if ord(c) == 0x15: # NAK
                if retries > self.RETRIES:
                    raise ComunicationError, "Fallo el envio del comando a la impresora luego de varios reintentos"
                # Reenvio el mensaje
                self._write( message )
                timeout = time.time() + self.WAIT_TIME
                retries +=1
            ...
            if c == chr(0x02):# STX - Comienzo de la respuesta
                ...
                while c != chr(0x03): # ETX (Fin de texto)
                    ...
                    if not c:
                        ...
                        if noreplyCounter > self.NO_REPLY_TRIES:
                            raise ComunicationError, "Fallo de comunicacion mientras se recibia la respuesta de la impresora."
                    else:
                        noreplyCounter = 0
                        reply += c
                ...
                if not self._checkReplyBCC( reply, bcc ):
                    ...
                    retries += 1
                    if retries > self.RETRIES:
                        raise ComunicationError, "Fallo de comunicacion, demasiados paquetes invalidos (bad bcc)."
                    continue
                elif reply[1] != chr( self._sequenceNumber ): # Los numero de seq no coinciden
                    # Reenvio el mensaje
                    self._write( message )
                    timeout = time.time() + self.WAIT_TIME
                    retries +=1
                    if retries > self.RETRIES:
                        raise ComunicationError, "Fallo de comunicacion, demasiados paquetes invalidos (mal sequence_number)."
                    continue
                else:
                    # Respuesta OK
                    break
...
##                          (1<<5, "Poco papel para comprobantes o tickets"),
    ...
    def _sendAndWaitAck( self, message, count = 0 ):
        if count > 10:
            raise ComunicationError, "Demasiados NAK desde la impresora. Revise la conexion."
        self._write( message )
        timeout = time.time() + self.WAIT_TIME
        while 1:
            if time.time() > timeout:
...
##                continue
            if c == chr(0x02):# STX - Comienzo de la respuesta
                ...
                if not self._checkReplyBCC( reply, bcc ):
                    ...
                    retries +=1
                    if retries > self.RETRIES:
                        raise ComunicationError, "Fallo de comunicacion, demasiados paquetes invalidos (bad sequenceNumber)."
                    continue
                else:
                    # Respuesta OK
                    self._write( self.ACK )
    ...
    def _sendCommand( self, commandNumber, fields, skipStatusErrors = False ):
        ...
        if reply[:5] == "REPLY":
            ...
            if errorClass:
                ...
                raise ProxyError( "Codigo de error desconocido: %s." % reply[7:] )
        else:
            raise ProxyError( "Respuesta no valida del servidor: %s." % reply )
...
if __name__ == "__main__":
    ...
    parser.add_option( "-p", "--port", action = "store", type = "string",
                       ...
                       help = "Puerto donde escucha el server, si no se indica, la comunicacion es por la entrada y salida estandar" )
    parser.add_option( "-i", "--ip", action = "store", type = "string",
                       dest = "ip", default = "",
                       help = "IP o Host donde escucha el server, si no se indica, la comunicacion es por la entrada y salida estandar" )
    ...
    parser.add_option( "-T", "--timeout", action = "store", type = "string",
                       ...
                       help = "Tiempo de espera antes de cancelar la conexion (en segundos). Default: 60 segundos" )
...
# SEND|0x2a|F|["N"]
# Envia el comando 0x2a, El "F" es para skipStatusErrors, y los parametros del comando: ["N"]
